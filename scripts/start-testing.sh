...
try {
    ...
    if ($cert) {
        $pfxPassword = ConvertTo-SecureString -String "<REMOVED_FROM_HISTORY>" -Force -AsPlainText
...
show_next_steps() {
    ...
    echo "   - Run: az keyvault certificate import --vault-name $KV_NAME --name azfw-intermediate-ca --file azfw-intermediate-ca.pfx --password '<REMOVED_FROM_HISTORY>'"
    echo ""
